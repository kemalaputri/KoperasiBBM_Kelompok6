...
// Jika belum login, tetap bisa akses beranda, tapi jika login role bukan user, lempar
if(isLoggedIn() && getUserRole() != 'user') { 
 ...
 $user_data = null;
if($id_user > 0) {
    $user_data = mysqli_fetch_assoc(mysqli_query($conn, "SELECT * FROM users WHERE id_user=$id_user"));
}
...
<head>
    ...
    <title>Beranda - Koperasi BBM</title>
    ...
    <div class="hero-content">
        <?php if(isLoggedIn() && $user_data): ?>
            <div class="user-greeting">
                <h2>Selamat Datang, <?php echo htmlspecialchars($user_data['nama_lengkap']); ?></h2>
                <p>Temukan kebutuhan sehari-harimu di Koperasi BBM.</p>
            </div>
            <div class="member-info">
                <?php 
                    if($user_data['jenis_anggota'] == 'Internal Sekolah') {
                        echo "Bagian: " . htmlspecialchars($user_data['bagian_internal']);
                    } else {
                        echo "Jenis Anggota: " . htmlspecialchars($user_data['jenis_anggota']);
                    }
                ?>
            </div>
        <?php else: ?>
            <h1>KOPERASI BINA BANGKIT MANDIRI</h1>
            <p>Pusat Layanan Belanja</p>
            <p style="font-size: 1rem; margin-top:5px; font-weight: 600;">SMKN 1 CIBINONG</p>
        <?php endif; ?>
    </div>
</section>
...
<section class="product-section">
    <div class="container">
        <h2 class="section-title">Belanja Produk</h2>
            ...
            ?>
                <div class="product-card" data-kategori="<?php echo $p['nama_kategori']; ?>">
                    <div class="product-img">
                        <?php if($p['gambar'] != 'default.jpg' && file_exists(__DIR__ . '/../uploads/produk/'.$p['gambar'])): ?>
                            <img src="<?php echo base_url(); ?>/uploads/produk/<?php echo $p['gambar']; ?>" alt="<?php echo $p['nama_produk']; ?>">
                        <?php else: ?>
